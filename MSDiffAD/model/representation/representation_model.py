...
class Mask(nn.Module):
    ...
    def forward(self, x, x1, intensity_):
        intensity_ = intensity_.squeeze(1)
        mask_bool_list = []
        mask_token_list = []
        mask_pos_list = []
        for p in range(x.shape[0]):
            pos_p = torch.argsort(intensity_[p, :], descending=True)  # 获取按强度排序的位置
            len_mask = 2  # 控制遮蔽的数量
            mask_pos = pos_p[torch.randperm(5)[0:len_mask]].to(device)  # 随机选择要遮蔽的位置
            mask_bool = torch.rand(x.shape[1]) > 1  # 生成随机的遮蔽布尔值
            mask_bool[mask_pos] = True  # 设置需要遮蔽的位置为True
            mask_bool = mask_bool.to(device)  # 将mask_bool移动到相应的设备上
            mask_bool_list.append(mask_bool.unsqueeze(0))

            mask_token = x[p, :][mask_pos]  # 获取原始x中对应位置的token
        ...
        # 对x和x1应用相同的mask
        mask_x = torch.masked_fill(x, mask_bool, 1)  # 对x进行遮蔽
        mask_x1 = torch.masked_fill(x1, mask_bool, 1)  # 对x1进行遮蔽
        ...
        return mask_x, mask_x1, mask_token_list, mask_pos_list
...
class MSDiffusion(nn.Module):
    ...
    def forward(self, input_id, aug_seq1, intensity):
        mask_x1, mask_x2, mask_token1, mask_pos1 = self.mask(input_id, aug_seq1, intensity)
        mask_x3, mask_token3, mask_pos3 = self.mask2(input_id, intensity)
        ...
        output3 = self.embedding(mask_x3)
        output3 = output3 + inten
        ...
        atten_mask3 = (mask_x3 == 0).unsqueeze(1).repeat(1, mask_x3.size(1), 1).unsqueeze(1)
        for transformer in self.transformer_blocks:
            output3 = transformer.forward(output3, atten_mask3)
        pool3 = torch.matmul(intensity, output3)
        pool3 = pool3 / intensity.shape[1]
        ...
        h_masked2 = torch.gather(output2, 1, mask_pos1)
        h_masked2 = self.activ2(self.linear(h_masked2))
        logits_lm2 = self.fc2(h_masked2)

        mask_pos3 = mask_pos3[:, :, None].expand(-1, -1, self.hidden)
        h_masked3 = torch.gather(output3, 1, mask_pos3)
        h_masked3 = self.activ2(self.linear(h_masked3))
        logits_lm3 = self.fc2(h_masked3)

        return logits_lm1, logits_lm2, mask_token1, pool1, pool2, logits_lm3, mask_token3, pool3
