...
class GaussianDiffusion(nn.Module):
    ...
    def __init__(
        ...
        super(GaussianDiffusion, self).__init__()  # 初始化 nn.Module
    ...
    def training_losses_seq2seq(self, model, t, input_ids_x, input_ids_mask, attention_mask, noise=None):
        ...
        model_out_x_start = self._x0_helper(model_output, x_t, t)[
            'pred_xstart']  # pred_xstart = model_output  这里这个model_out_x_start就是前边的model_output
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, **kwargs):
        super().__init__(**kwargs)  # 继承自 nn.Module 的初始化
        self.use_timesteps = set(use_timesteps)
        self.timestep_map = []
        self.original_num_steps = len(kwargs["betas"])

        base_diffusion = GaussianDiffusion(**kwargs)  # Initialize the base diffusion model
        last_alpha_cumprod = 1.0
        new_betas = []
        for i, alpha_cumprod in enumerate(base_diffusion.alphas_cumprod):
            if i in self.use_timesteps:
    ...
    def p_mean_variance(self, model, x, t, attention_mask, clip_denoised, denoised_fn):
        # 调用父类的 p_mean_variance
        return self.base_diffusion.p_mean_variance(self._wrap_model(model), x, t, attention_mask, clip_denoised,
                                                   denoised_fn)

    def training_losses(self, model, t, input_ids, input_ids_mask, attention_mask):
        # 调用父类的 training_losses
        return self.base_diffusion.training_losses(self._wrap_model(model), t, input_ids, input_ids_mask,
                                                   attention_mask)

    def rec_predict(self, model, *args, **kwargs):
        # 调用父类的 rec_predict
        return self.base_diffusion.rec_predict(self._wrap_model(model), *args, **kwargs)
