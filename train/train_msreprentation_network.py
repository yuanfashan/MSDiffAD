from model.representation.trainner import Trainer
...
class DataGenerator2:
    def __init__(self, max_len=100):
        ...
        # 加载数据
        train_ref, msms1, precursor1, smiles1 = ParseOrbitrap('D:/Projects/MSDiffAD/data/ob_test_query.pickle')
        print(len(smiles1))
        ...
        # 生成数据加载器
        self.train_dataloader = DataLoader(self.dataset, batch_size=64, shuffle=True)
        self.valid_dataloader = DataLoader(self.dataset, batch_size=64, shuffle=False)  # 可以根据需要修改
        self.test_dataloader = DataLoader(self.dataset, batch_size=64, shuffle=False)  # 可以根据需要修改
...
def main():
    ...
    # 测试数据
    test_ref, test_msms1, test_precursor1, test_smiles1 = ParseOrbitrap('D:/Projects/MSDiffAD/data/ob_train_ref.pickle')
    test_query, test_msms2, test_precursor2, test_smiles2 = ParseOrbitrap('D:/Projects/MSDiffAD/data/ob_train_query.pickle')
    test_ref, _ = MakeTrainData(test_msms1, test_precursor1, max_len)
    test_query, _ = MakeTrainData(test_msms2, test_precursor2, max_len)
    ...
    for epoch in range(epochs):
        ...
        train_time.append(t)
        top1_accuracy, best_accuracy = trainer.test_step(best_accuracy,test_ref, test_query, test_smiles1, test_smiles2)
        # top1_accuracy = trainer.test_step(train_ref, train_query, smiles1, smiles2)
